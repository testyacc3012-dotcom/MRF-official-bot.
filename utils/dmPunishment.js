...
const { issueAppealInvite, banFromAppealServer } = require('./appealInvites');
 ...
 * a new punishment generates a fresh one. The user is also banned from the
 * appeal server at this point so any old invite or lingering membership
 * cannot be used — /whitelist removes that ban when an administrator is
 * ready to let them appeal.
 *
...
async function dmPunishment(user, { action, guildName, reason, duration, client, includeAppeal = true }) {
  ...
  if (includeAppeal) {
    let link = null;
    if (client) {
      // Issue the new single-use invite (revokes whatever they had before),
      // then immediately ban them from the appeal server so no old invite or
      // lingering membership gives them access. /whitelist reverses this ban
      // when an administrator decides to restore their appeal access.
      link = await issueAppealInvite(client, user.id);
      await banFromAppealServer(client, user.id);
    }
